...
export const SchedulePage: React.FC<SchedulePageProps> = ({ activities, students, groups, transactions, onAddActivity, onUpdateActivity, onUpdateAttendance, onUpdateFeePayment, onDeleteActivity, onAddTransaction, onUpdateTransaction, currentUser, onRefresh }) => {
  ...
  const visibleActivities = useMemo(() => {
    const filteredByType = activities.filter(a => a.type !== 'MONTHLY_EVALUATION');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2 w-full sm:w-auto">
              <button onClick={() => setViewMode(viewMode === 'DAILY' ? 'UPCOMING' : 'DAILY')} className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${viewMode === 'UPCOMING' ? 'bg-primary-100 text-primary-700' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}>
                {viewMode === 'UPCOMING' ? 'Próximos Jogos/Treinos' : 'Ver Próximos'}
              </button>
              {viewMode === 'DAILY' && (
                  <>
          ...
          {viewMode === 'DAILY' && <button onClick={handleGoToday} className="text-sm text-primary-600 font-medium hover:bg-primary-50 px-3 py-1.5 rounded-lg">Hoje</button>}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
            {(viewMode === 'DAILY' ? dailyActivities : upcomingActivities).length > 0 ? (viewMode === 'DAILY' ? dailyActivities : upcomingActivities).map(a => {
            ...
            {selectedActivity ? (
                <div className="bg-white rounded-xl border border-gray-100 flex flex-col shadow-sm">
                    ...
                    {selectedActivity.description && (
                        <div className="px-4 py-3 bg-yellow-50 text-xs text-yellow-800 border-b border-yellow-100 italic">
                            <span className="font-bold not-italic text-yellow-900 uppercase tracking-wide mr-1">Observação:</span>
                            {selectedActivity.description}
                        </div>
                    )}
                             ...
                             <div className="flex items-center mt-3">
                                 <input type="checkbox" id="askTransport" className="mr-2 w-4 h-4 text-yellow-600 focus:ring-yellow-500 rounded cursor-pointer" checked={newActivity.askTransport || false} onChange={e => {
                                     ...
                                     setNewActivity({
                                         ...
                                         presentationLocation: isChecked ? newActivity.presentationLocation : undefined,
                                         presentationTime: isChecked ? newActivity.presentationTime : undefined
                                     });
                                 }} />
      ...
      {notifyModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-md p-6 border-t-4 border-blue-500">
            <div className="flex justify-between items-center mb-4"><h3 className="text-lg font-black flex items-center gap-2 text-gray-800 uppercase tracking-tighter"><Megaphone className="w-5 h-5 text-blue-600" /> Disparos WhatsApp</h3>{!notifyIsRunning && <button onClick={() => setNotifyModalOpen(false)}><X className="text-gray-400" /></button>}</div>
