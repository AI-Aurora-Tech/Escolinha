...
function App() {
  ...
  const handleAddStudent = async (studentData: Omit<Student, 'id'>) => {
    setIsLoading(true);
    const payload = { 
        name: studentData.name, birth_date: studentData.birthDate, rg: studentData.rg, cpf: studentData.cpf, 
        phone: studentData.phone, medical_expiry: studentData.medicalCertificateExpiry, photo_url: studentData.photoUrl,
        address: studentData.address, guardian: studentData.guardian, 
        plan_id: studentData.planId === '' ? null : studentData.planId,
        group_ids: studentData.groupIds || [], active: studentData.active, documents: studentData.documents
    };
    const { error } = await supabase.from('students').insert([payload]);
    if (error) alert(`Erro ao salvar: ${error.message}`); else await fetchData(true);
    setIsLoading(false);
  };
  ...
  const handleUpdateStudent = async (student: Student) => {
    ...
    if (error) alert(`Erro ao atualizar: ${error.message}`); else await fetchData(true);
    setIsLoading(false);
  };
  ...
  const handleUpdateActivity = async (a: Activity) => {
    setIsLoading(true);
    const payload = { title: a.title, activity_type: a.type, fee: a.fee, location: a.location, presentation_time: a.presentation_time, opponent: a.opponent, home_score: a.home_score, away_score: a.away_score, scorers: a.scorers, group_id: a.groupId, participants: a.participants, date: a.date, start_time: a.startTime, end_time: a.endTime, recurrence: a.recurrence, attendance: a.attendance, fee_payments: a.fee_payments };
    const { error } = await supabase.from('activities').update(payload).eq('id', a.id);
    if (!error) await fetchData(true);
    setIsLoading(false);
  };

  // Added missing handlers for App.tsx
                ...
                <div className="p-8">
                    {activeLoginTab === 'EMAIL' ? (
                        <form onSubmit={(e) => { e.preventDefault(); setIsAuthenticated(true); setCurrentUser({id:'1', name:'Admin', email:'admin@martinica.com', role: UserRole.ADMIN, avatar:''}); }} className="space-y-4">
                            ...
                            <input type="email" placeholder="E-mail" className="w-full border rounded-lg p-3" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} />
                            <input type="password" placeholder="Senha" className="w-full border rounded-lg p-3" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} />
                            <button className="w-full bg-primary-600 text-white font-bold py-3 rounded-lg">Entrar</button>
                        </form>
                    ) : (
                        <form onSubmit={(e) => { e.preventDefault(); setIsAuthenticated(true); setCurrentUser({id:'2', name:'Responsável', email:'', role: UserRole.RESPONSAVEL, avatar:'', cpf: loginCpf}); }} className="space-y-4">
                            <input type="text" placeholder="CPF do Responsável" className="w-full border rounded-lg p-3" value={loginCpf} onChange={e => setLoginCpf(e.target.value)} />
                            <button className="w-full bg-primary-600 text-white font-bold py-3 rounded-lg">Acessar Portal</button>
                        </form>
                    )}
                </div>
            </div>
