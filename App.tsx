...
function App() {
    ...
    try {
        ...
        if (activitiesData) {
             setActivities(activitiesData.map((a: any) => ({
                 ...
                 // NORMALIZAÇÃO CRÍTICA: Extrai apenas YYYY-MM-DD para compatibilidade com filtro da Agenda
                 date: a.date ? (a.date.includes('T') ? a.date.split('T')[0] : a.date) : '', 
                 startTime: a.start_time || '00:00',
                 endTime: a.end_time || '00:00',
  ...
  const handleAddStudent = async (s: Omit<Student, 'id'>) => {
    ...
    if (!error && data) {
        ...
        fetchData(); // Refresh to generate tuiton if needed
    } else {
  ...
  // Fix: Implemented handleBatchAddStudents to handle bulk Excel imports
  const handleBatchAddStudents = async (sList: Omit<Student, 'id'>[]) => {
    setIsLoading(true);
    try {
        const payloads = sList.map(s => ({
            name: s.name, 
        ...
        if (!error) {
            ...
            alert("Erro ao importar lote de alunos.");
        }
    } catch (err) {
        console.error("Unexpected error in handleBatchAddStudents:", err);
    } finally {
        setIsLoading(false);
    }
  };
  ...
  const handleAddActivity = async (a: any) => { 
    ...
    const { data } = await supabase.from('activities').insert([{
        title: a.title, activity_type: a.type, fee: feeValue, location: a.location, group_id: a.groupId,
        participants: a.participants, start_time: a.startTime, end_time: a.endTime, date: a.date, recurrence: a.recurrence
    }]).select();
    if (data) fetchData();
  };
  const handleUpdateActivity = async (a: Activity) => { await supabase.from('activities').update({
    title: a.title, activity_type: a.type, fee: a.fee, location: a.location, group_id: a.groupId,
    participants: a.participants, start_time: a.startTime, end_time: a.endTime, date: a.date, 
    attendance: a.attendance, fee_payments: a.feePayments, opponent: a.opponent, home_score: a.homeScore,
    away_score: a.awayScore, scorers: a.scorers, presentation_time: a.presentationTime
  ...
  const handleUpdateAttendance = async (aid: string, sid: string) => { 
    ...
    await supabase.from('activities').update({ attendance: newAttendance }).eq('id', aid); fetchData(); 
  };
  const handleUpdateFeePayment = async (aid: string, sid: string) => {
    const activity = activities.find(a => a.id === aid);
    if(!activity) return;
    const currentFees = activity.feePayments || [];
    const newFees = currentFees.includes(sid) ? currentFees.filter(id => id !== sid) : [...currentFees, sid];
    await supabase.from('activities').update({ fee_payments: newFees }).eq('id', aid); fetchData();
  };
  ...
  return (
    <div className="flex bg-gray-50 min-h-screen">
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 w-full">
        {isLoading ? <div className="flex justify-center mt-20"><Loader2 className="animate-spin text-primary-600 w-10 h-10" /></div> : renderContent()}
      </main>
    </div>
  );
}
